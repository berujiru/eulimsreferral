...
<div class="pstcsample-form">

    <div class="image-loader" style="display: hidden;"></div>

    <?php $form = ActiveForm::begin(); ?>
    <?php if(empty($model->pstc_sample_id)): ?>
    <div class="row">
    ...
    <?= $form->field($model, 'sampling_date')->widget(DateTimePicker::classname(), [
            'options' => [
    ...
    <?= $form->field($model, 'sample_description')->textarea(['rows' => 6]) ?>

    <div class="form-group" style="padding-bottom: 3px;">
        <div style="float:right;">
            <?= Html::submitButton($model->isNewRecord ? 'Add' : 'Update', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary','id'=>'btn-update']) ?>
...
$this->registerJs("$('#saved_templates').on('change',function(){
    var id = $('#saved_templates').val();
        $.ajax({
            //url: '".Url::toRoute("/pstc/pstcsample/getlist_template?template_id='+id+'")."',
            url: '".Url::toRoute("/pstc/pstcsample/getlist_template")."',
            dataType: 'json',
            method: 'GET',
            //data: {id: $(this).val()},
            data: {template_id: id},
            success: function (data, textStatus, jqXHR) {
                $('#pstcsample-sample_name').val(data.name);
                $('#pstcsample-sample_description').val(data.description);
